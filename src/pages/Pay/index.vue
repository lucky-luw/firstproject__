...
<script>
import QRCode from 'qrcode' //生成二维码的包
  export default {
    name: 'APay',
    data(){
      return {
    ...
    methods:{
      ...
      async open(){
        ...
        console.log(url);
        this.$confirm(`<img src=${url} />`,"请扫码支付", {
          confirmButtonText: '我已支付成功',
          cancelButtonText: '取消支付',
          center: true,
          dangerouslyUseHTMLString:true,
          beforeClose:(type,instance,done)=>{
            //type:区分取消还是确定的按钮  instance：当前组件实例  done：关闭弹出框的方法
            if(type=='cancel') {
        ...
        //设置定时器的目的多次发请求，访问是否支付成功
      if(!this.timer){
        this.timer=setInterval(async ()=>{
          let result=await this.$API.reqPayStatus(this.orderId)
          if(result.code==200){//当支付成功时
            clearInterval(this.timer);//清除定时器

            this.timer=null
            
            this.code=result.code;//将支付状态码保存
            this.$router.push('/paySuccess')//路由跳转
