<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="people in addressInfo" :key="people.id" >
        <span class="username" :class="{selected:people.isDefault==1}">{{people.consignee}}</span>
        <p @click="changeDefault(people.id)">
          <span class="s1">{{people.fullAddress}}</span>
          <span class="s2">{{people.phoneNum}}</span>
          <span class="s3" v-show="people.isDefault==1">默认地址</span>
        </p>
      </div>
      <div class="line"></div>
...
export default {
  ...
  methods:{
    changeDefault(defaultId){
      this.addressInfo.forEach(item=>{
        if(item.id==defaultId) item.isDefault=1
   ...
   async submitOrder(){
      ...
      // 在组件中发请求，不用vuex，可以直接在主入口文件收集API
    
        let result=await this.$API.reqSubmitOrder(this.orderInfo.tradeNo,data)
        if(result.code==200){
          this.tradeSuccessId=result.data
          this.$router.push({
            name:'pay',
            query:{
             orderId: this.tradeSuccessId
            }
          })
          //this.$router.push('pay?orderId='+this.tradeSuccessId)
        }else {
